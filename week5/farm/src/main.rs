...
fn num_agent(remaining_num: Arc<Mutex<VecDeque<u32>>>) {
    loop {
        let num = {
            let mut remaining_num_ref = remaining_num.lock().unwrap();
            remaining_num_ref.pop_front()
        };
        match num {
