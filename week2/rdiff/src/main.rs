...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m+1{
        Grid::set(&mut c, i, 0, 0);
    }
    for j in 0..n + 1{
        Grid::set(&mut c, 0, j, 0);
    }

    for i in 0..m{
        for j in 0..n{
            if seq1[i] == seq2[j]{
                let c_ij : usize = c.get(i, j).unwrap();
                c.set(i + 1, j + 1, c_ij + 1);
            }
            else {
                let max_ : usize = max(c.get(i + 1, j).unwrap(), c.get(i, j + 1).unwrap());
                c.set(i + 1, j + 1, max_);
            }
        }
    }
...
fn main() {
    ...
    //unimplemented!();
    // Be sure to delete the #[allow(unused)] line above
    let line1 = read_file_lines(filename1).expect("Failed to read the file1");
    let line2 = read_file_lines(filename2).expect("Failed to read the file2");

    let lcs_table = lcs(&line1, &line2);

    print_diff(&lcs_table, &line1, &line2, line1.len(), line2.len());
}
